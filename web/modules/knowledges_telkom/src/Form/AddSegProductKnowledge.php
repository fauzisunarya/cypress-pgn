...
class AddSegProductKnowledge extends FormBase
{
    public function getFormId()
    {
        return 'add_seg_product_knowledge';
    }
    ...
    public function buildForm(array $form, FormStateInterface $form_state)
    {
        ...
        $form['category'] = [
            ...
            '#ajax' => [
                ...
                'wrapper' => 'seg_pknowledge_container'
            ]
        ];
        ...
        $form['seg_pknowledge_container'] = [
            '#type' => 'container',
            '#attributes' => [
                'id' => 'seg_pknowledge_container',
            ],
        ];

        $form['seg_pknowledge_container']['seg_pknowledge_parent'] = [
            '#type' => 'entity_autocomplete',
    ...
    public function validateForm(array &$form, FormStateInterface $form_state)
    {
        ...
        $id = $form_data['seg_pknowledge_parent'];
        $data = Node::load($id);
        ...
        if (is_null($data)) {
            $form_state->setErrorByName('pkt_pknowledge_parent', 'Please pick Product Knowledge first');
        }
    }

    public function submitForm(array &$form, FormStateInterface $form_state)
    {
        $seg_id = Drupal::routeMatch()->getParameter('seg_id');
        $form_data = $form_state->getValues();
        $seg_pknowledge_parent = $form_data['seg_pknowledge_parent'];
        $destination = !empty($form_data['destination_hidden']) ? $form_data['destination_hidden'] : "/product-catalog/{$seg_id}/product-knowledges";
        $response = new RedirectResponse(getenv('APP_URL'). "/node/add/segment_product_knowledge?destination={$destination}&seg_id={$seg_id}&seg_pknowledge_parent={$seg_pknowledge_parent}");
        $response->send();
    }

    public function handleFilter(array $form, FormStateInterface $form_state)
    {
        return $form['seg_pknowledge_container'];
    }
}
