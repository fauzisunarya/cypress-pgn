...
class Testing2 {
  ...
  public function data() {
    ...
    return array(
      "blockID"    => 'testing-02',
      "title"      => 'Testing 2',
      "screenshot" => 'testing-02/testing-02.jpg',
      ...
      <div :class="{'container': !style.fullwidth.value, 'container-fluid': style.fullwidth.value }">
        <div :class="`content-spacer`" :style="{'padding': `${style.padding.value.top}px ${style.padding.value.right}px ${style.padding.value.bottom}px ${style.padding.value.left}px`}">
          <div :class="`section-header mb4 ${data.alignment.value}`">
            <h2 class="section-title" :style="`--font-size:${data.title_size.value}px`">
              <InlineEditor v-model="data.title.value"/>
            </h2>
            <div class="section-subtitle">
              <InlineEditor v-model="data.subtitle.value"></InlineEditor>
            </div>
          </div>

            <Sortable v-model="data.features.value" class="feature-grid columns">
              <SortableItem class="column is-4" v-for="(item, index) in data.features.value" :key="`item-${index}`" :list="data.features.value" :index="index">
                <div class="icl-feature icl-feature--style-1">
                  <div class="icl-feature__card">
                    ...
                    <span :class="`icl-feature__icon ${data.icon_style.value}`" :style="{fontSize:data.icon_size.value+'px'}">
                      <Icon v-model="item.icon.value" />
                    </span>
  ...
  static function render($id, $data, $style) {
    ...
    <div
      ...
      <div class="icl-section__bg" style="<?php echo $background_style; ?>">
        ...
        <?php echo modules::run('api/render/render_divider', $style->divider->value ); ?>
      </div>
...
$arrayDevCode[] = $testing2->data();
$arrayDevTemplate[] = $testing2->editorTemplate();