<template>
  <div v-if="posts.length">
    <div v-if="mode == 'list'" class="ukm-posts">
      <article class="ukm-post--list" v-for="post in posts" :key="post.id">
        <div v-if="post.attachment" :class="`ukm-post--image --size-${image_size || '1by1'}`">
          <figure>
            <img :src="post.attachment" alt="">
          </figure>
        </div>
        <div :class="'ukm-post--detail ' + post_detail_classname">
    ...
    <div v-else class="ukm-posts layout-grid" :data-columns="column">
      <a href="#" class="ukm-post--grid" v-for="post in posts" :key="'post' + post.id">
        <div class="ukm-post--inner">
          <div :class="`ukm-post--image --size-${post.attachment ? image_size : '1by1'}`">
            <figure>
              <div v-if="post.attachment" class="angga-post10-detail-bg"></div>

              <img v-if="post.attachment" :src="post.attachment" alt="">

              <div :class="'ukm-post--detail ' + post_detail_classname">
                <div class="date">
                  <small class="angga-color-primary">{{formatDate(post.created_date)}}</small>
                </div>
                <h2
                  :style="{fontSize: `${title_size}px`}"
                  :class="post.attachment ? 'has-text-white' : 'has-text-grey'">
                  <a href="#">
                    {{post.title}}
                  </a>
                </h2>
...
export default{
  ...
  props: {
    mode                  : { type: String },
    post_detail_classname : { type: String },
