 ...
 * @endcode
 */
$settings['hash_salt'] = 'jccbDBCziZh6_xoP3WMr3V_CvHXpapMZpesy8RWKcVyKz9PfEeEyqKd0WE_pq3pUw8v41K0Qbg';
...
# }
$config_directories['../config/sync'] = '../config/sync';

// error notice
$config['system.logging']['error_level'] = $_ENV['ERROR_NOTICE'] ?? getenv('ERROR_NOTICE');

# Load environment
$environment  = $_ENV['ENVIRONMENT'] ?? getenv('ENVIRONMENT');
$base_path    = $app_root . '/' . $site_path;
$servicesFile = $base_path . "/services.{$environment}.yml";
$settingsFile = $base_path . "/settings.{$environment}.php";
...
$databases['default']['default'] = array(
    'database' => $_ENV['DATABASE_NAME'] ?? getenv('DATABASE_NAME'),
    'username' => $_ENV['DATABASE_USER'] ?? getenv('DATABASE_USER'),
...
$settings['s3fs.access_key'] = $_ENV['S3_ACCESS_KEY'] ?? getenv('S3_ACCESS_KEY');
$settings['s3fs.secret_key'] = $_ENV['S3_SECRET_KEY'] ?? getenv('S3_SECRET_KEY');
// $config['s3fs.settings']['bucket'] = 'storagetelkomcms';
$config['s3fs.settings']['region'] = $_ENV['S3_REGION'] ?? getenv('S3_REGION');

$config['s3fs.settings']['hostname'] = $_ENV['S3_HOSTNAME'] ?? getenv('S3_HOSTNAME');
