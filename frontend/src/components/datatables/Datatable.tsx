...
import { Stack, TextField, Card, CardContent, Typography, CardActions, Button, Collapse, List, ListItem, ListItemText, IconButton, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, ButtonGroup } from "@mui/material";
...
function TableView(props: {
  ...
  const { data, page, rowCount, rowTotal, keyword, sort, reload, setSort, setData, setPage, setRowCount, setRowTotal, setKeyword, setReload } = useDatatable();
  useEffect(() => {
    const fetch = async () => {
      setData({
        ...data,
    ...
    <div style={{ height: '100vh', width: '100%' }}>
      <Stack direction="row" justifyContent={'space-between'}>
        ...
        <Stack direction="row" spacing={1}>
          <Button variant="contained" color="info" size="small" onClick={setReload}><Iconify width={14} icon="fa6-solid:arrow-rotate-right" /></Button>
          <Button variant="contained" color="info" size="small"><Iconify icon="fa6-solid:filter" width={14}/></Button>
          <ExportExcel variant="contained" color="info" size="small" addonParam={props.addonParam} load={props.load} responseDataLocation={props.responseDataLocation}><Iconify width={14} icon="fa6-solid:download" /></ExportExcel>
          {(saveOption) && (<Button variant="contained" color="info" size="small" onClick={() => handleClickOpenForm()}><Iconify icon="fa6-solid:plus" width={14} /></Button>)}
        </Stack>
        {/* </ButtonGroup> */}
        <TextField size="small" onChange={(val: any) => 
          { 
            if(props.searchAction !== undefined){
      ...
      {(!isDesktop && mobileOptions != undefined) && (
        <Stack sx={{ width: "100%" }} spacing={2}>
          {(data.data?.length > 0) && (data.data).map((val: any, i: number) => (
